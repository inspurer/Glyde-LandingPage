...
**Scroll drives the length while the section is pinned.** `.s2ManualScroller` is a runway one viewport taller than the content, with `.s2ManualPin` stuck to the top of it; scrolling through that extra height does not move the content, it steps the value 0.1 through 0.9, and the page carries on once the far end is reached. The design's default 0.5 holds only until the runway is entered for the first time; the hop to 0.1 as it engages is the transition, not a jump. After that it never returns — scrolling back up above the section leaves the value at 0.1, because springing back to 0.5 as the section clears the top undoes the last step the visitor just watched. 0.5 is an opening state, not somewhere to come back to.
...
- **There is no `wheel` handler.** It existed to turn scroll into value changes and did it by `preventDefault`ing every wheel event over the control, which is the desktop half of the same hijack.
